...
describe('Given the productmovements controller', () => {
  ...
  describe('When the deleteById method is called', () => {
    ...
    test('Then, if a Bearer formed authorization and a query param are included in the request, the deleteById method of the repo should have been called and a json respond and its status should be send when data is received', async () => {
      mockReqWithBearerFormedAuthorization.params.id = 'mockId';
      await controllerWithResp.deleteById(
        mockReqWithBearerFormedAuthorization,
        mockResp,
        next
      );
      expect(repoMockWithResp.deleteById).toHaveBeenCalled();
      expect(mockResp.json).toHaveBeenCalled();
      expect(mockResp.status).toHaveBeenCalled();
    });
    test('Then, if a Bearer formed authorization is included in the request but a query param is not included in the request, an error should be thrown', async () => {
